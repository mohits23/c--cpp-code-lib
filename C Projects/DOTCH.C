...
void move_red_token(int noondice,int color) // 'color' is the respected color of the token to be passed.
{
   int i,clr;
...
void starter_game()
{
   ...
   char ch;
	...
	n=roll_dice();
	dice(n);
	 move_yellow_token(n,YELLOW);
     }
     else if( strike==2 )
     {
	n=roll_dice();
	dice(n);
	 move_blue_token(n,BLUE);
     }
     else if( strike==3 )
     {
	n=roll_dice();
	dice(n);
	 move_green_token(n,GREEN);
     }
     else if( strike==4 )
     {
	n=roll_dice();
	dice(n);
	 move_red_token(n,RED);
     }   */
